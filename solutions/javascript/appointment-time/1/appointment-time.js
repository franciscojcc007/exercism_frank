...
export function timeBetween(timestampA, timestampB) {
  ...
  const numA = Number(dateA) / secondTransformation
  const numB = Number(dateB) / secondTransformation
...
export function isValid(appointmentTimestamp, currentTimestamp) {
  const dateAppointment = new Date(appointmentTimestamp)
  const dateCurrent= new Date(currentTimestamp)

  const numA = Number(dateAppointment) 
  const numB = Number(dateCurrent) 
